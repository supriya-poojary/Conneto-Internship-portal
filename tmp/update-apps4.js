...
const styleStart = companyContent.indexOf('<style>');
const styleEnd = companyContent.indexOf('</style>') + 8;
const sidebarStart = companyContent.indexOf('<div class="sidebar-overlay" id="sidebarOverlay"></div>');
const sidebarEnd = companyContent.indexOf('</aside>') + 8;

if (styleStart === -1 || sidebarStart === -1) {
    console.error("Could not find style or sidebar in company.ejs");
    process.exit(1);
}

let companyStyles = companyContent.substring(styleStart + 7, styleEnd - 8);
let sidebarHtml = companyContent.substring(sidebarStart, sidebarEnd);
...
const newAppsHtml = tick + `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
fs.writeFileSync(appsPath, eval('newAppsHtml'));
